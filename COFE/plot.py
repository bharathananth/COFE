...
def plot_circular_ordering(adata, filename=None, **kwargs):
    ...
    gs = mp.gridspec.GridSpec(1, 3, width_ratios=[1.0, 1.0, 1.0])
    ax = fig.add_subplot(gs[0, 0])
    sns.scatterplot(
        x=adata.obsm["X_scpca"][:, 0],
        y=adata.obsm["X_scpca"][:, 1],
        ax=ax,
        legend=False,
        c=["0.3"] * adata.n_obs,
    )

    Y = adata.X @ adata.varm["PCs"] / adata.uns["scpca"]["d"]
    sns.scatterplot(
        x=Y[:, 0],
        y=Y[:, 1],
        ax=ax,
    ...
    circle = mp.patches.Circle(
        (0, 0), radius=1, alpha=0.5, ec="grey", ls=(0, (5, 3)), fill=False, lw=2.0
    )
    ax.add_patch(circle)
    ax.set_aspect("equal")
    ...
    df = pd.DataFrame(
        {
            "t": adata.obs["phase"] / adata.uns["period"],
            "CPC1": adata.obsm["X_scpca"][:, 0],
            "CPC2": adata.obsm["X_scpca"][:, 1],
        }
    )
    ...
    if "time" in adata.obs_keys():
        sns.scatterplot(
            x=(adata.obs["time"] % adata.uns["period"]),
            y=adata.obs["phase"],
            ax=ax,
            edgecolor="black",
            c=["0.3"] * adata.n_obs,
...
def plot_cv_run(adata, **kwargs):
    ...
    df1 = pd.DataFrame(
        np.hstack(
            (adata.uns["scpca"]["s_choices"][:, None], adata.uns["scpca"]["runs"])
        ),
        columns=["s", "cv_mean", "cv_se"],
    ).set_index("s")
    fig = mp.pyplot.figure(**kwargs)
    gs = mp.gridspec.GridSpec(1, 1)
    ax = fig.add_subplot(gs[0, 0])
    ax.errorbar(
        df1.index, df1["cv_mean"], yerr=df1["cv_se"], fmt="ko-", ecolor="dimgray"
    )
...
def plot_diagnostics(adata, **kwargs):
    ...
    ax = fig.add_subplot(gs[0, 1])
    bin_edges = np.histogram_bin_edges(adata.X.mean(axis=0), bins=50)
    bin_edges = np.append(np.insert(bin_edges, 0, [-np.inf]), [np.inf])
    ax.hist(
        adata.X.mean(axis=0), bins=bin_edges, cumulative=-1, histtype="step", lw=2.0
    )
    ax.grid(True)
    ax.set_xlabel("Mean value of feature")

    ax = fig.add_subplot(gs[0, 2])
    bin_edges = np.histogram_bin_edges(1 / adata.X.std(axis=0), bins=50)
    bin_edges = np.append(np.insert(bin_edges, 0, [-np.inf]), [np.inf])
    ax.hist(
        1 / adata.X.std(axis=0),
        bins=bin_edges,
        cumulative=True,
        histtype="step",
        lw=2.0,
...
def plot_markers(adata, translate=False, **kwargs):
    ...
    adata : AnnData object
        with the outputs of cross_validate function
    ...
    df = pd.DataFrame(adata.varm["PCs"], index=adata.var_names, columns=["X", "Y"])
    ...
    ax = sns.catplot(
        x="value", y="index", data=df, col="CPC", kind="bar", aspect=0.5, **kwargs
    )
    ax.set_xlabels("Loadings")
    ax.set_ylabels("Features")


def print_markers(adata, translate=False):
    ...
    df.reset_index(level=0, inplace=True, names="index")
