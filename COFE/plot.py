...
def plot_circular_ordering(adata, filename=None, **kwargs):
    ...
    sns.scatterplot(x=adata.obsm["X_scpca"][:, 0], y=adata.obsm["X_scpca"][:, 1], 
                    ax=ax, legend=False, c=['0.3'] * adata.n_obs)

    Y = adata.X @ adata.varm["PCs"] / adata.uns["scpca"]["d"]
    sns.scatterplot(x=Y[:, 0], y=Y[:, 1], ax=ax, hue=adata.obs["time"], 
    ...
    df = pd.DataFrame({'t': adata.obs["phase"]/adata.uns["period"], 
    ...
    if "time" in adata.obs_keys():
        sns.scatterplot(x = (adata.obs["time"] % adata.uns["period"]), 
                        y = adata.obs["phase"], 
...
def plot_cv_run(adata, **kwargs):
    ...
    df1 = pd.DataFrame(np.hstack((adata.uns["scpca"]["s_choices"][:, None], adata.uns["scpca"]["runs"])), columns=["s", "cv_mean", "cv_se"]).set_index("s")
    fig = mp.pyplot.figure(**kwargs)
...
def plot_diagnostics(adata, **kwargs):
    ...
    ax = fig.add_subplot(gs[0, 0]);
    ax.hexbin(x=adata.X.mean(axis=0), y=np.sqrt(adata.X.var(axis=0)), bins='log',
                cmap='viridis')
    ...
    ax.hist(adata.X.mean(axis=0), bins=bin_edges, cumulative=-1, histtype='step', 
            lw=2.0)
    ax.grid(True)
    ax.set_xlabel("Mean value of feature")
    ...
    bin_edges = np.append(np.insert(bin_edges, 0, [-np.inf]), [np.inf])
    ax.hist(1/adata.X.std(axis=0), bins=bin_edges, cumulative=True, 
...
def print_markers(adata, translate=False, index_name="index"):
    ...
    df = pd.DataFrame(adata.varm["PCs"], index=adata.var_names, 
                      columns=["X","Y"])
    df = df.loc[(df!=0).any(axis=1)].sort_index()
