...
def preprocess_data(
    ...
    X_test_ = (
        adata.X[~adata.obs["train"], :].copy() if any(adata.obs["train"]) else None
    )
    ...
    if impute is not None:
        if isinstance(impute, float) and impute < 100.0 and impute > 0.0:
            upper_bound = np.percentile(
                X_train_, 100 - impute / 2, axis=0, keepdims=True
            )
            lower_bound = np.percentile(X_train_, impute / 2, axis=0, keepdims=True)
            X_train_ = np.maximum(np.minimum(X_train_, upper_bound), lower_bound)
            if X_test_ is not None:
                X_test_ = np.maximum(np.minimum(X_test_, upper_bound), lower_bound)
...
def cross_validate(
    ...
    if s_choices is None:
        ...
        rss_cv = np.ma.array(rss_cv, mask=np.isnan(rss_cv)).reshape(
            (len(rss_cv) // (repeats * K), repeats, K)
        )
        ...
        if any(count_nan > len(cv_indices) / 4):
            warn(
                "Too many runs did not converge for s={}. CV results might be "
                "unreliable. Try increasing max_iter or reducing the tolerances.".format(
                    s_choices[count_nan > len(cv_indices) / 4]
                )
            )

        mask_size = np.array([m.shape[0] for m in cv_indices]).reshape(repeats, K)

        mask_size = np.ma.array(
            np.repeat(mask_size[None, :, :], s_choices.shape[0], axis=0),
...
def predict_time(X_test, cv_results, true_times=None, period=24.0):
    ...
    """
    V = cv_results["SLs"]
    pred_phase, mape_value, circ_corr = calculate_mape(
        X_test @ V, true_times, period=period
    )
...
def estimate_phase(adata, train=False):
    ...
    if "X_scpca" in adata.obsm.keys():
        ...
        if "time" in adata.obs_keys():
            ...
            diff_offsets_cw = [
                (np.median(np.abs(_delta(scaled_time[ind] - angles[ind], d))), d)
            ...
            if (
                diff_offsets_cw[best_offset_ind_cw][0]
                < diff_offsets_acw[best_offset_ind_acw][0]
            ):
                adjusted_opt_angles = (
                    angles + diff_offsets_cw[best_offset_ind_cw][1]
                ) % 1
                mape_value = diff_offsets_cw[best_offset_ind_cw][0]
            else:
                adjusted_opt_angles = (
                    -angles + diff_offsets_acw[best_offset_ind_acw][1]
                ) % 1
...
def _shuffled_checkerboard(size, K, repeats, seed):
    ...
    for r in range(repeats):
        ...
        for i in range(K):
            ...
            row_ind = np.array(
                [(row_one + nrow * i + i) % nrow for i in range(ncol)], dtype="int"
            ).flatten()
            col_ind = np.array(
                [i * np.ones(row_one.shape) for i in range(ncol)], dtype="int"
            ).flatten()
            return_list.append(
                np.ravel_multi_index(
                    (row_permute[row_ind], col_permute[col_ind]), (nrow, ncol)
                )
            )
    return return_list
...
def _fischer_circ_corr(phi_1, phi_2):
    ...
    den = np.sqrt(
        np.sum(np.sin(ph_1 - mean_ph_1) ** 2) * np.sum(np.sin(ph_2 - mean_ph_2) ** 2)
    )
    rho = num / den
    return rho
...
def _calculate_cv(adata, s, tol, tol_z, max_iter, cv_ind, scale_by_features, seed):
    ...
    adata = sparse_cyclic_pca_masked(
        adata=adata,
        mask=mask,
        s=s,
        tol=tol,
